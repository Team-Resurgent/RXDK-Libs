...
| 2 | `libxapi` | `xsndtrk` | `XAddSoundtrack`, `XAddSongToSoundtrack` — the soundtrack *write* side. Retail keeps it in a separate opt-in `xsndtrk.lib` (the *read* side, `XOpenSoundtrackSong` etc., is already in xapilib and matches us); we fold the write side into libxapi in its own pull-on-reference TU (`k32/xsndtrkw.c`). Same additive-enhancement class as the keyboard row below: strictly a superset (a title that linked `xsndtrk.lib` still links; nothing needs the separate archive), and non-users pay nothing because the write TU is only pulled when referenced. |
| 3 | `libxapi` | `xkbd`/`xkbdd` | `XInputDebugInitKeyboardQueue`, `XInputDebugGetKeystroke`, `XDEVICE_TYPE_DEBUG_KEYBOARD_TABLE`. **Not a parity break — an enhancement that preserves parity.** Retail confines keyboard/mouse to the opt-in dev-time `xkbd.lib`; RXDK folds them into `libxapi` so they work in RELEASE builds too. This is strictly additive (a debug→release promotion): every 5849 title still links exactly as before — those symbols resolve — and non-debug titles simply *gain* the capability. So do not split it back out for archive-name parity; the divergence is only which archive the symbol sits in, and the behavior is a superset of 5849. Work here goes further the same way, toward ordinary (non-`Debug`-gated) input paths. |
