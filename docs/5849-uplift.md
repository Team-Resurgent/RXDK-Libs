...
**61 public APIs absent from our libs** (was 64 before `IXACTSoundSource_SetPitch`/
`GetStatus`/`SetFilter`/`GetProperties` and `IDirectSound{Buffer,Stream}_GetVoiceProperties`
were recovered — the dsound pair had been hidden by the audit's dsound.lib-ends-in-'d' bug):
...
| `xact.h` | 6 | `EnableHeadphones`, `GetRealtimeData`, `SetI3dl2Listener`, `GetSoundCueProperties`, `SelectVariation`, `IXACTSoundSource_SetMode`. Recovery in progress from `xacteng.lib` — see `5849-xact-api-recovery.md`. |
| `DSound.h` | 1 | `XAudioSetEffectData`. |
