...
| libxonline | `xonline.lib` (+ `xonlinen`/`xonlines`, `uix.lib`) | 🔶 partial | 5849 public `xonline.h` adopted; leak client compiles+links on libxnet(LIBO). **REAL now:** the UIX **logon feature** (`src/uix5849.cpp`): `StartFeature(UIX_LOGON_FEATURE)` enumerates stored accounts (`XOnlineGetUsers`), shows a controller-driven account picker rendered through the title's `ITitleUIPlugin`/`ITitleFontRenderer`, runs the leak's real `XOnlineLogon` task pumped in `DoWork`, reports `UIX_EXIT_LOGON_*` via `GetExitInfo`, then keeps pumping the logon task to hold the Live connection (what Insignia talks to). `XOnlineSilentLogon` is also real (wraps `XOnlineLogon` with the first stored account). UIX gaps (TODO): friends/players screens, skin rendering (text-only via the font renderer), multi-user/guest logon, passcode entry, notifications UI. `src/uplift5849.cpp` still carries the documented-stub tail (stats/storage/friends/signature/matchmaking/mutelist/messaging/arbitration/competition/teams/presence/query/offerings — each fails cleanly). |
