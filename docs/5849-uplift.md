...
| libxonline | `xonline.lib` (+ `xonlinen`/`xonlines`, `uix.lib`) | ❌ leak | Newer logon-state API (`XONLINE_LOGON_STATE`, `XONLINE_MAX_LOGON_STATE_SERVICES`) + winsock prelude. **Compile-only value — Live servers dead, untestable.** |
...
| libdsound | `dsound.lib` | 🔶 partial | Added `DSVOICEPROPS` (5849, recovered via cvdump) — needed by the 5849 xact.h. Still: a few newer constants (`DSBPLAY_SYNCHPLAYBACK`, `DSMIXBIN…`). |
| libxapi | `xapilib.lib` | ❌ leak | |
...
|---|---|---|
| `xvoice.lib` | XHV — Xbox high-level voice chat | Candidate port (samples need `xhv.h`/`xvoice.h`, 7 samples). |
...
| `uix.lib` | Xbox UI helper (Live) | Folded conceptually with xonline; header `uix.h` present. |
...
_Last updated: 2026-08-03. Tooling: `cvdump` (RXDK-Tools). See also the samples/middleware memory note._
