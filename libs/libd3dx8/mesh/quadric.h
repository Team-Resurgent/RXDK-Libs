...
/* RXDK: picolibc's <math.h> already provides isnanf (as a macro over the
   __builtin); defining our own inline isnanf(float) collides with it (the macro
   mangles the definition, and #undef-ing it then clashes with picolibc's
   `int isnanf(float)` decl on return type). Use picolibc's when present. */
#ifndef isnanf
