...
/* RXDK: every OutputDebugString call below passes a wide (L"...") literal, but
   without UNICODE the macro resolves to the ANSI OutputDebugStringA(LPCSTR).
   Route to the W variant (libxapi thunks it to A) for this file. */
#ifdef OutputDebugString
#undef OutputDebugString
#endif
/* The L"" literals are const wchar_t*; OutputDebugStringW takes LPCWSTR
   (const WCHAR* = const unsigned short*). Both are 16-bit on this target, so
   reinterpret the pointer. */
#define OutputDebugString(s) OutputDebugStringW((LPCWSTR)(s))
