...
/* RXDK: AddToDynamicArray is defined in tri3mesh.inl, which tri3mesh.h includes
   AFTER this file -- so under clang's two-phase lookup it isn't visible at the
   point the templates below reference it (and ADL can't find it: the args are
   builtin pointers). Forward-declare the template here. */
template<class T> BOOL AddToDynamicArray(T **ppBase, const T& obj, UINT *pcNum, UINT *pcMax);
...
class CMeshStatus
{
        /* RXDK: SMSListElem is a template; clang rejects befriending it with a
           bare `struct` elaborated-specifier. Befriend the matching spec. */
        friend struct SMSListElem<UINT_IDX>;
public:
            ...
            for (iNeighbor = 0; iNeighbor < 3; iNeighbor++)
            {
                if (pdwNeighbors[iNeighbor] != UNUSED32)
                {
                    ...
                    else
                    {
                        ...
                        /* RXDK: rgdwNeighbors is `const DWORD*`; without the cast the two args deduce
                           FindEdge's UINT_IDX as `const DWORD` vs `DWORD` -> no match. FindEdge only reads. */
                        iNeighborEdgeBack = FindEdge((DWORD*)&rgdwNeighbors[rgiFaceRemapInverse[wNeighbor] * 3], (DWORD)rgiFaceRemapInverse[iFace]);
