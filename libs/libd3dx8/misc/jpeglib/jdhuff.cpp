...
/* RXDK: was `#ifdef _X86_`. This block is the hand-written MASM (__asm{})
   fast Huffman decoder (decode_mcu_fast / jpeg_huff_decode_fast). It is DEAD
   CODE -- jinit_huff_decoder below always assigns the portable C `decode_mcu`
   (its `#ifdef _X86_` dispatch is itself `#if 0`'d), so these are never
   referenced. clang cannot parse the MASM `(type [reg]).field` operand syntax,
   so we exclude the block from the build (no behavior change). */
#if 0
