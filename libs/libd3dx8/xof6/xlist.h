...
public:
    ...
    /* RXDK: const ref so a char* arg can bind when T=const char* (MSVC bound a
       char* lvalue to non-const const-char*& here; clang rejects). Add only
       reads tSrc. */
    int Add(const T & tSrc)
    {
        if (m_cUsed == m_cTotal) {
            m_cTotal += m_cChunk;
