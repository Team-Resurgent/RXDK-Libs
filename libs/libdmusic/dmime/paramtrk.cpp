...
CParamControlTrack::Clone(MUSIC_TIME mtStart, MUSIC_TIME mtEnd, IDirectMusicTrack** ppTrack)
{
            ...
            pObject = pObject->GetNext())
    {
                ...
                pParam = pParam->GetNext())
        {
            ...
            // RXDK/clang: hoist pCurveStart (used to seed the next loop); MSVC for-scope leak.
            DMUS_IO_PARAMCONTROLTRACK_CURVEINFO *pCurveStart;
            for (pCurveStart = param.curves;
                    (pCurveStart < param.curvesEnd) && (pCurveStart->mtEndTime < mtStart);
                    ++pCurveStart)
            {}
            // Then, find the curve whose start time is after mtEnd.
            // RXDK/clang: hoist pCurveEnd (used after loop); MSVC for-scope leak.
            DMUS_IO_PARAMCONTROLTRACK_CURVEINFO *pCurveEnd;
            for (pCurveEnd = pCurveStart;
