...
/*
 * CWavTrack -- track that schedules and plays DirectSound wave data on
...
HRESULT CWavTrack::ComputeVariations(WaveStateData* pSD)
{
    ...
    int i;
    for (i = 0; i < MAX_WAVE_VARIATION_LOCKS; i++)
    {
                                  ...
                                  BOOL fClockTime)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        for (DWORD dwPart = 0; pScan && dwPart < m_dwPChannelsUsed; pScan = pScan->GetNext(), dwPart++)
        {
            ...
            // RXDK/clang: hoist dwSyncPart (used after loop); MSVC for-scope leak.
            DWORD dwSyncPart;
            for (dwSyncPart = 0; dwSyncPart < Variations.dwPChannelsUsed; dwSyncPart++)
            {
