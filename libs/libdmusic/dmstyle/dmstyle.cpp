...
CDirectMusicPattern* CDirectMusicPattern::Clone(MUSIC_TIME mtStart, MUSIC_TIME mtEnd, BOOL fMotif)
{
    ...
    if (pNewPattern)
    {
        ...
        // RXDK/clang: hoist i -- the second loop below reuses it (MSVC for-scope leak).
        int i;
        for (i = 0; i < pNewPattern->m_wNumMeasures; i++)
        {
            ...
            MUSIC_TIME& rmtNextCommand)
{
    ...
    // RXDK/clang: MSVC accepted implicit-int here; make the type explicit.
    static int aMatchType[5] =
...
bool IsSimple(DMUS_CHORD_PARAM& Chord, int& rCount, int& rSixSeven)
{
    ...
    // RXDK/clang: hoist n -- the "count the total notes" loop below reuses it
    // (MSVC for-scope leak).
    int n;
    for (n = 0; n < Chord.bSubChordCount; n++)
    {
                                               ...
                                               PatternTrackState* pState)
{
    ...
    // RXDK/clang: hoist i -- reused by the part loop below (MSVC for-scope leak).
    short i;
    for (i = 0; i < MAX_VARIATION_LOCKS; i++)
    {
...
//    TListItem<DirectMusicPartRef>* pPartRef = m_PartRefList.GetHead();
    ...
    //  DirectMusicPartRef& rPartRef = pPartRef->GetItemValue();
        ...
        else
        {
            ...
            // RXDK/clang: hoist nV -- reused by later loops in this function
            // (MSVC for-scope leak).
            short nV;
            for (nV = 0; nV < 32; nV++)
            {
