                           ...
                           TransitionConstraint& rTransition)
{
    ...
    if (pCompFragment)
    {
        ...
        while (pfragmentScan && !(rTransition.dwFlags & DMUS_TRANSITIONF_LAST_FOUND))
        {
            ...
            int i; // RXDK/clang: hoisted (used after loop); MSVC for-scope leak.
...
BYTE TransposeNote(BYTE bMIDI, int nInterval, DMUS_CHORD_PARAM& rChord)
{
    ...
    int nSemitone; // RXDK/clang: hoisted (used after loop); MSVC for-scope leak.
    for (nSemitone = 0; nSemitone < 24; nSemitone++)
    {
...
void ScaleMisses(BYTE bTone, DWORD dwScale, BYTE& rFlats, BYTE& rSharps)
{
    ...
    int i; // RXDK/clang: hoisted -- reused by the loops below (MSVC for-scope leak).
    for (i = 0; i <= bTone; i++)
    {
        if (dwScale & (1 << i))
        {
