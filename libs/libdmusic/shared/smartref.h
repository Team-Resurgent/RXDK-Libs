...
namespace SmartRef
{
	...
	public:
		...
		// RXDK/clang two-phase lookup: m_fFail is not a member of Vector (only
		// m_pT/m_size/m_capacity exist) -- a leak bug MSVC never caught because it
		// only instantiates a template member when used, and this operator is
		// unused. Clang binds the non-dependent name at definition time and errors.
		// Vector never actually reports failure through bool (callers check
		// AccessTo()/Grow()), so return false to keep this unused method compilable.
		...
		bool Grow(UINT size)
			{
				if (size > m_size)
				{
					if (size > m_capacity)
					{
						// RXDK/clang: hoist the loop var out of the for-init; the
						// leak relied on MSVC's pre-standard for-scope leak to use
						// `capacity` after the loop (lines below + m_capacity=capacity).
						UINT capacity;
						for (capacity = m_capacity ? m_capacity : 1;
								capacity < size;
								capacity *= 2)
						{}
						T *pT = new T[capacity];
