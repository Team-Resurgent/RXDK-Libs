...
// RXDK/clang: the THROW macro passes a temporary CException; MSVC bound it to a
// non-const reference (a well-known MSVC extension). Take a const reference so
// the temporary binds under standard C++.
extern EXPORTDEF void APINOT ThrowDebugException(unsigned int iLine,
                                                 char * szFile,
                                                 const CException & ecE );

extern EXPORTDEF void APINOT ThrowException(const CException & ecE,
                                            unsigned dummy);
