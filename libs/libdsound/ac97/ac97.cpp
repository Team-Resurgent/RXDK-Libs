...
/* RXDK: variadic no-op; clang errors on extra args to a 0-arity macro (MSVC C4002). */
#define DPF_AC97(...)
...
)
{
    ...
    {
        KeInitializeDpc(&m_dpc, (PKDEFERRED_ROUTINE)AciInterruptDpcHandler, this);
    }
    ...
    if(SUCCEEDED(hr))
    {
        ...
        KeInitializeInterrupt(&m_Interrupt, (PKSERVICE_ROUTINE)AciInterruptServiceRoutine, this, ulInterruptVector, Irql, LevelSensitive, TRUE);
    ...
    if(SUCCEEDED(hr))
    {
        m_HalShutdownData.NotificationRoutine = (PHAL_SHUTDOWN_NOTIFICATION)AciShutdownNotifier;
...
)
{
    ...
    do
    {
        ...
        KeSynchronizeExecution(&m_Interrupt, (PKSYNCHRONIZE_ROUTINE)AciSynchronizationRoutine, this);
