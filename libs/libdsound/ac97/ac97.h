...
/*
 * Interface for the AC'97 codec device driver (CAc97Device): the channel
 * identifiers and the class that drives the MCP AC'97 controller registers
 * and DMA buffer lists.
 */
...
namespace DirectSound
{
    ...
    private:
        // HAL callbacks. RXDK: the kernel invokes ISR/DPC/sync routines __stdcall;
        // clang defaults to cdecl -> stack corruption when they fire. Pin __stdcall.
        static BOOLEAN __stdcall AciInterruptServiceRoutine(PKINTERRUPT pInterrupt, LPVOID pvContext);
        static void __stdcall AciInterruptDpcHandler(PKDPC pdpc, LPVOID pvDeferredContext, LPVOID pvSystemArgument1, LPVOID pvSystemArgument2);
        static void __stdcall AciShutdownNotifier(PHAL_SHUTDOWN_REGISTRATION pHalShutdownData);
        static BOOLEAN __stdcall AciSynchronizationRoutine(LPVOID pvContext);
    };
    ...
    __inline BOOLEAN __stdcall CAc97Device::AciInterruptServiceRoutine(PKINTERRUPT pInterrupt, LPVOID pvContext)
    {
    ...
    __inline void __stdcall CAc97Device::AciInterruptDpcHandler(PKDPC pdpc, LPVOID pvDeferredContext, LPVOID pvSystemArgument1, LPVOID pvSystemArgument2)
    {
    ...
    __inline void __stdcall CAc97Device::AciShutdownNotifier(PHAL_SHUTDOWN_REGISTRATION pHalShutdownData)
    {
    ...
    __inline BOOLEAN __stdcall CAc97Device::AciSynchronizationRoutine(LPVOID pvContext)
    {
        CAc97Device *           pThis   = (CAc97Device *)pvContext;
