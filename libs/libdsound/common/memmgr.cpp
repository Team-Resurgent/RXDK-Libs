...
/* RXDK: route ExAllocatePoolWithTag -> ExAllocatePool (dropping the debug tag),
   the same workaround libxapi/support/inc/ntos/ex.h uses for the USB stack. The
   stale prebuilt xboxkrnl.lib mis-maps ExAllocatePoolWithTag (@15): on HW it
   returns its 2nd arg (the tag), so PoolAlloc below then ZeroMemory's the tag
   address ('boSD' = 0x626F5344) and access-violates. ExAllocatePool (@14) is
   correctly mapped (+ facaded by libxapi). Done after dscommon.h so the kernel
   prototype parses first; only the call site is rewritten. Proper fix:
   regenerate xboxkrnl.lib -- tracked task task_37451d8f. */
#ifdef ExAllocatePoolWithTag
#undef ExAllocatePoolWithTag
#endif
#define ExAllocatePoolWithTag(cb, tag) ExAllocatePool(cb)
