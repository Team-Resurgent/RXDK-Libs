...
static __inline float ds_alog(const float x)
{
	...
	/* RXDK: renamed local 'one' -> 'flOne'; shared guiddef.h has `#define one 1`,
	   which clang expands inside this declaration (MSVC's guiddef.h has no such macro). */
	float flOne = 1;
	float dummy;
	float n;
	...
	fadd flOne;							// 2^(x/log10(2)-n) = 2^(x/log10(2)) * 2^(-n)
#ifdef	_MSC_VER
...
)
{
	...
	for(int i = 0; i < 4; i++)					// RXDK: re-declare 'i' (MSVC for-scope leak)  // D05 to D08
	{
