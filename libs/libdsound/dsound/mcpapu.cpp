...
)
{
	...
	if(SUCCEEDED(hr))
    {
        KeInitializeDpc(&m_dpcInterrupt, (PKDEFERRED_ROUTINE)ApuInterruptDpcRoutine, this);
        KeInitializeTimer(&m_tmrDeferredCommandsHigh);
        KeInitializeDpc(&m_dpcDeferredCommandsHigh, (PKDEFERRED_ROUTINE)DeferredCommandDpcRoutine, this);
    }
    ...
    if(SUCCEEDED(hr))
    {
        ...
        KeInitializeInterrupt(&m_Interrupt, (PKSERVICE_ROUTINE)ApuInterruptServiceRoutine, this, ulInterruptVector, irql, LevelSensitive, TRUE);
    ...
    if(SUCCEEDED(hr))
    {
        m_HalShutdownData.NotificationRoutine = (PHAL_SHUTDOWN_NOTIFICATION)ApuShutdownNotifier;
...
)
{
    ...
    while(TRUE)
    {
        KeSynchronizeExecution(&m_Interrupt, (PKSYNCHRONIZE_ROUTINE)GetInterruptStatusCallback, this);
...
BOOLEAN __stdcall              // RXDK: kernel-invoked (KeSynchronizeExecution) -> stdcall
