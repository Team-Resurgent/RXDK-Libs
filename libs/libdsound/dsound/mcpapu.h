...
namespace DirectSound
{
    ...
    private:
        // RXDK: kernel-invoked callbacks must be __stdcall (the kernel's ISR/DPC/
        // sync dispatch calls them stdcall; clang defaults to cdecl -> stack
        // corruption when they fire). Pinned with the explicit keyword.
        static BOOLEAN __stdcall ApuInterruptServiceRoutine(PKINTERRUPT pInterrupt, LPVOID pvContext);
        static void __stdcall ApuInterruptDpcRoutine(PKDPC pDpc, LPVOID pvDeferredContext, LPVOID pvSystemContext1, LPVOID pvSystemContext2);
        static BOOLEAN __stdcall GetInterruptStatusCallback(LPVOID pvContext);
        static void __stdcall DeferredCommandDpcRoutine(PKDPC pdpc, LPVOID pvDeferredContext, LPVOID pvSystemContext1, LPVOID pvSystemContext2);
        static void __stdcall ApuShutdownNotifier(PHAL_SHUTDOWN_REGISTRATION pHalShutdownData);
    };
    ...
    __inline BOOLEAN __stdcall CMcpxAPU::ApuInterruptServiceRoutine(PKINTERRUPT pInterrupt, LPVOID pvContext)
    {
        return (BOOLEAN)((CMcpxAPU *)pvContext)->ServiceApuInterrupt();
    }

    __inline void __stdcall CMcpxAPU::ApuInterruptDpcRoutine(PKDPC pDpc, LPVOID pvDeferredContext, LPVOID pvSystemContext1, LPVOID pvSystemContext2)
    {
        ((CMcpxAPU *)pvDeferredContext)->ServiceApuInterruptDpc();
    }

    __inline void __stdcall CMcpxAPU::DeferredCommandDpcRoutine(PKDPC pdpc, LPVOID pvDeferredContext, LPVOID pvSystemContext1, LPVOID pvSystemContext2)
    {
        ((CMcpxAPU *)pvDeferredContext)->ServiceDeferredCommandsHigh();
    }

    __inline void __stdcall CMcpxAPU::ApuShutdownNotifier(PHAL_SHUTDOWN_REGISTRATION pHalShutdownData)
    {
