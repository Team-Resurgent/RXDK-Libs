...
/*
 * Streaming (packet-at-a-time) front end over the ported ffmpeg WMA v1/v2
 * decoder. wmabridge.h's XactWmaDecode decodes a whole buffer in one call,
 * which suits XACT's decode-on-load wave banks but not the XMO decoders
 * (XWmaDecoderCreateMediaObject and friends), which must hand back a bounded
 * amount of PCM per Process() call while the title keeps rendering. Pure C,
