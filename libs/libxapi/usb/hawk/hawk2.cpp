...
/*
 * Major entry points of the Hawk driver, the Xbox Communicator (voice headset)
 * USB class driver:
 *
 *   Entries required by the USB core stack:
 *       HawkInit, HawkAddDevice, HawkRemoveDevice
 *   Entries exported to titles (declared in xvoice.h):
 *       XVoiceCreateMediaObject, XVoiceCreateMediaObjectEx
 *
 * Retail ships this driver in xvoice.lib; RXDK carries it with the other USB
 * class drivers in libxapi because the class-driver list is a static table
 * here (rxdk_usb_class_pointers in usbd.cpp) rather than the .XPP$Class*
 * linker-section walk MS LINK provided.
 */
...
//------------------------------------------------------------------------------
//  Pool allocation helpers.  The original defined a file-local operator new
//  over ExAllocatePoolWithTag; Clang rejects a static global operator new, so
//  allocate explicitly (tag 'kwah') and placement-construct where a vtable is
//  involved.
//------------------------------------------------------------------------------
static __inline PVOID HawkAllocPool(SIZE_T cb)
{
    return ExAllocatePoolWithTag(cb, 'kwah');
}

struct HAWK_PLACEMENT {};
inline void * __cdecl operator new(size_t, void *pv, HAWK_PLACEMENT) { return pv; }
inline void __cdecl operator delete(void *, void *, HAWK_PLACEMENT) {}
...
HawkInit (IUsbInit *pUsbInit)
{
    ...
    //table entries (look in the tables above) - old Hawk FW enumerates as
    //USB_DEVICE_CLASS_AUDIO instead of XBOX_DEVICE_CLASS_AUDIO_DEVICE.
...
void XHawkMediaObject::InitializeClass(ULONG ulMicrophoneCount, ULONG ulHeadphoneCount)
{
    ...
    PHAWK_STREAMING_RESOURCES pStreamingResourceArray = NULL;
    ULONG i;
    ...
    if(ulTotalStreamingResources)
        pStreamingResourceArray = (PHAWK_STREAMING_RESOURCES)
            HawkAllocPool(sizeof(HAWK_STREAMING_RESOURCES) * ulTotalStreamingResources);


    sm_pHeadphones = (XHawkMediaObject *)HawkAllocPool(sizeof(XHawkMediaObject) * ulPortCount);
    sm_pMicrophones = (XHawkMediaObject *)HawkAllocPool(sizeof(XHawkMediaObject) * ulPortCount);
    sm_pRateIndices = (PUCHAR)HawkAllocPool(sizeof(UCHAR) * ulPortCount);
    sm_pRateIndexRefCounts = (PUCHAR)HawkAllocPool(sizeof(UCHAR) * ulPortCount);
    ...
    RIP_ON_NOT_TRUE_WITH_MESSAGE(!ulTotalStreamingResources || pStreamingResourceArray, "XInitDevices failed due to insufficent memory.  There is no graceful recovery.");

    //
    //  Run the constructors (sets the vtable and default state).
    //
    for(i = 0; i < ulPortCount; i++)
    {
        new (&sm_pHeadphones[i], HAWK_PLACEMENT()) XHawkMediaObject();
        new (&sm_pMicrophones[i], HAWK_PLACEMENT()) XHawkMediaObject();
    }
    ...
    for(i=0; i < ulTotalStreamingResources; i++)
    {
...
//
//  Public name for the callback-taking factory (xvoice.h): identical to
//  XVoiceCreateMediaObjectInternal, which is the name dsoundp.h declares.
//
...
XVoiceCreateMediaObjectEx(
      ...
      IN DWORD dwPort,
      IN DWORD dwMaxAttachedPackets,
      IN LPCWAVEFORMATEX pwfxFormat,
      IN PFNXMEDIAOBJECTCALLBACK    pfnCallback,
      IN PVOID                      pvContext,
      OUT XMediaObject **ppXmediaObject
      )
{
    return XVoiceCreateMediaObjectInternal(
          XppDeviceType,
          dwPort,
          dwMaxAttachedPackets,
...
XVoiceCreateMediaObject(
      ...
      IN DWORD dwPort,
      IN DWORD dwMaxAttachedPackets,
      IN LPWAVEFORMATEX pwfxFormat  OPTIONAL,
      OUT XMediaObject **ppXmediaObject
      )
/*++
    Routine Description:
...
--*/
{
    ...
    {
        DbgPrint("HAWK: SetSampleRate open-default-ep failed 0x%08x\n", usbdStatus);
    ...
    USB_BUILD_CONTROL_TRANSFER(
        ...
        (USHORT) 0x0100 | ucRateIndex, //Rate Index to set, see the communicator spec.
        HAWK_VENDOR_FEATURE_SAMPLE_RATE,
        0); //Transfer length is zero

    usbdStatus = m_pDevice->SubmitRequest(&Urb);
    if(USBD_ERROR(usbdStatus))
        DbgPrint("HAWK: SetSampleRate(%u) vendor xfer failed 0x%08x\n", ucRateIndex, usbdStatus);
...
/*++
    Routine Description:
        Sends a command to hawk to change the AGC setting.  This routine opens the
        default pipe. Sends a vendor specific SetFeature to change the AGC.  Then closes
        the default pipe.  The routine is synchronous, which means it must drop to below
        DISPATCH_LEVEL.
    Parameters:
        ucAGC - AGC on/off.
...
--*/
{
    ...
    //
    //  Send the SET_FEATURE(AGC) command.
    //
    ...
    USB_BUILD_CONTROL_TRANSFER(
        ...
        USB_VENDOR_COMMAND | USB_COMMAND_TO_INTERFACE | USB_HOST_TO_DEVICE,
        USB_REQUEST_SET_FEATURE,
        (USHORT)ucAGC, //ON/OFF, see the communicator spec.
        (USHORT)HAWK_VENDOR_FEATURE_AGC,
        0); //Transfer length is zero

    usbdStatus = m_pDevice->SubmitRequest(&Urb);
    if(USBD_ERROR(usbdStatus))
        DbgPrint("HAWK: SetAGC(%u) vendor xfer failed 0x%08x\n", ucAGC, usbdStatus);
...
--*/
{
    ...
    //
    PHAWK_PACKET_CONTEXT pPacketContextArray  = (PHAWK_PACKET_CONTEXT)
        HawkAllocPool(sizeof(HAWK_PACKET_CONTEXT) * ulMaxAttachedPackets);
    ...
    {
        DbgPrint("HAWK: iso open ep=0x%02x maxpkt=%u failed 0x%08x\n",
                 m_bEndpointAddress, m_wMaxPacket, status);
        ExFreePool(pPacketContextArray);
...
void XHawkMediaObject::FreeStreamingResources (void)
{
    ...
    //
    ExFreePool(m_pStreamingResources->m_pContextArray);
