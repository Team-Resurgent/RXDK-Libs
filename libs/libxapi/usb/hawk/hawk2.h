...
/*
 * Header file for the Hawk driver — the Xbox Communicator (voice headset)
 * USB class driver. Kernel mode.
 *
 * The communicator is one USB device with two vendor-class (0x78) interfaces:
 * a microphone (iso IN) and a headphone (iso OUT). Each interface enumerates
 * as its own IUsbDevice and surfaces to titles as an XMediaObject created by
 * XVoiceCreateMediaObject[Ex] (XDEVICE_TYPE_VOICE_MICROPHONE / _HEADPHONE).
 */
...
#define _XAPI_
#define _KERNEL32_
#define NODSOUND  //so we can use the private version (dsoundp.h)
extern "C" {
#include <ntos.h>
}
#include <ntrtl.h>
...
#include <usb.h>
#include <xapidrv.h>
#include <xobjbase.h>   // STDMETHOD/THIS_/PURE for the XMediaObject interface

// dsoundp.h's 3D-listener structs name D3DXVECTOR3 (a d3dx8math class, layout =
// three floats). This driver never touches those fields, so a layout-compatible
// stand-in avoids dragging the whole d3dx8 umbrella into the USB stack.
...
/*
 * Tolerate a SET_FEATURE(SampleRate) failure when the requested rate is the
 * 16 kHz default, for fixed-rate communicator firmware (retail xhawk.lib was
 * built with this on).
 */
#define HAWK_SUPPORT_FIXED_RATE 1
...
#define HAWK_MAX_SAMPLES_PER_USB_FRAME      24  //The highest supported rate is 24 kHz
...
//*****************************************************************************
//  HAWK_WRAP_BUFFER used to wrap a transfer across two media packets
//*****************************************************************************
...
//*****************************************************************************
//  HAWK_TRANSFER_CONTEXT represents an outstanding Isoch Transfer
//*****************************************************************************
...
// XVoiceCreateMediaObject / XVoiceCreateMediaObjectInternal are declared by
// dsoundp.h (the private header); the public xvoice.h additionally names the
// Internal variant XVoiceCreateMediaObjectEx.
...
class XHawkMediaObject : public XMediaObject
{
    ...
    //
    //  The XVoiceCreateMediaObject factory method, and the
    //  USB interface methods, need to be friends.  They
    //  all would be members, but they must be C callable.
    //  Signatures must match dsoundp.h's declarations exactly.
    //
