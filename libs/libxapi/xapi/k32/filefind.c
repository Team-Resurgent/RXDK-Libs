    ...
    )
{
    ...
    {
        ULONG nameBytes = DirInfo->FileNameLength;
        ULONG maxNameBytes = (MAX_PATH - 1) * sizeof(OCHAR);

        if (nameBytes > maxNameBytes) {
            nameBytes = maxNameBytes;
        }

        RtlCopyMemory(lpFindFileData->cFileName, DirInfo->FileName, nameBytes);
        lpFindFileData->cFileName[nameBytes / sizeof(OCHAR)] = OBJECT_NULL;
    }
