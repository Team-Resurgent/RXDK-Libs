...
--*/
{
    ...
    m_Devices = (IUsbDevice *)USBD_AllocateMemory(sizeof(IUsbDevice) * m_NodeCount, MODULE_POOL_TAG);
    for (ULONG nodeIndex = 0; nodeIndex < m_NodeCount; nodeIndex++)
    {
        RtlZeroMemory(&m_Devices[nodeIndex], sizeof(IUsbDevice));
        m_Devices[nodeIndex].m_Type = UDN_TYPE_UNUSED;
        m_Devices[nodeIndex].m_NextFree = UDN_INVALID_NODE_INDEX;
