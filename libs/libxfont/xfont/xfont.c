...
//
// Ported from the leaked Xbox SDK source (private/windows/directx/dxg/xfont)
// into RXDK-Libs. This pass covers bitmap fonts only: XFONT_OpenBitmapFont
// (disk-file loading) is deliberately NOT ported -- its only caller was this
// file, so it's removed here rather than left as a dangling declaration.
// XFONT_OpenBitmapFontFromMemory/XFONT_OpenDefaultFont/XFONT_TextOut/
// XFONT_TextOutToMemory are the supported entry points. TrueType support
// (XFONT_OpenTrueTypeFont) stays excluded via the public header's own
// #ifdef XFONT_TRUETYPE guard -- simply not defining that macro anywhere in
// this build is sufficient, no header edits needed.
//
// History:
//
