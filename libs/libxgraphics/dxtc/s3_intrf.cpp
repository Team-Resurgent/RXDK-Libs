                            ...
                            for ( iy = 0 ; iy < 4 ; iy++, lpCur += dwSrcPitch - 4 * bpp )
                            {
                                if (iy<blockHeight)         // skip if outside
                                {
                                    ...
                                    for (ix=0;ix<4;ix++,lpCur+=bpp)
                                    {
                                        ...
                                        // pack it...
                                        if (ix<blockWidth)
                                            val=(val>>4) | ( (unsigned short)(floor(( ( dwCurTexel & fi.dwAMask ) >> aShift ) / float( fi.dwAMask >> aShift )* 15.0f + 0.5f )) << 12 );  /* RXDK: (unsigned short) cast -- clang rejects the multi-word functional-cast form */
                                        else
                                            val>>=4;
                                    }
                                    // write it out
