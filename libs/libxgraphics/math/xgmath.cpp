...
#define fRcp fY02
    ...
    // untouched on failure (e.g. the PushBuffer sample inverts an uninitialized matrix
    // via the D3DXMatrixInverse->XGMatrixInverse macro and depends on its modelView
    // surviving), so validate every element is finite and only then commit; otherwise the
    // matrix is effectively non-invertible -> return NULL.
    XGMATRIX m;
