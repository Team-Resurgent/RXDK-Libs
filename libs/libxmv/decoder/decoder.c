...
#define XMV_MAX_AUDIO_STREAMS 8
#define XMV_MAX_DIMENSION     1920

static HRESULT XMVValidateVideoDataHeader
(
    const XMVVideoDataHeader *pVideoDataHeader,
    DWORD BytesRead
)
{
    if (BytesRead < sizeof(XMVVideoDataHeader))
    {
        return E_FAIL;
    }

    if (pVideoDataHeader->MagicCookie != XMV_MAGIC_COOKIE
        || pVideoDataHeader->FileVersion != XMV_FILE_VERSION)
    {
        return E_FAIL;
    }

    if (!pVideoDataHeader->RequiredBufferSize
        || pVideoDataHeader->RequiredBufferSize > MAX_BUFFER_SIZE)
    {
        return E_FAIL;
    }

    if (pVideoDataHeader->ThisPacketSize < 4096
        || pVideoDataHeader->ThisPacketSize > pVideoDataHeader->RequiredBufferSize)
    {
        return E_FAIL;
    }

    if (pVideoDataHeader->AudioStreamCount > XMV_MAX_AUDIO_STREAMS)
    {
        return E_FAIL;
    }

    if (pVideoDataHeader->Width > XMV_MAX_DIMENSION
        || pVideoDataHeader->Height > XMV_MAX_DIMENSION)
    {
        return E_FAIL;
    }
...
)
{
    ...
    hr = XMVValidateVideoDataHeader(pVideoDataHeader, BytesRead);
    if (FAILED(hr))
    {
        goto Error;
    }
