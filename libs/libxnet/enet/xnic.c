...
/* RXDK: the kernel invokes ISR/DPC callbacks __stdcall; clang defaults to cdecl
   -> stack corruption when the NIC interrupt/DPC fires. Pin __stdcall. */
PRIVATE VOID __stdcall NicInterruptDpc(PKDPC, PVOID, PVOID, PVOID);
...

{
    ...
    DbgPrint("xnet-init:    NicInitialize: PhyInitialize...\n");
    status = PhyInitialize(FALSE, NULL);
    DbgPrint("xnet-init:    PhyInitialize done (0x%08x)\n", (unsigned)status);
    csr->mii_cs |= MIICS_APEN;
    if (!NT_SUCCESS(status)) goto err;
