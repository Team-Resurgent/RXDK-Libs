...
//
/* RXDK: was `__inline`. clang in C mode follows C99 inline semantics and does
   NOT emit an out-of-line copy of a plain `__inline` function, so the many
   INLINE helpers in the xnet headers (SysAlloc, XnetAlloc0, SockLock, Tcb*, ...)
   would be undefined at link. `static __inline` gives each TU its own copy. */
...
/* RXDK: on a dev kit these are the debug monitor's (stdcall) uncached
   alloc/free; pin __stdcall so calls through the pointer don't corrupt the stack. */
extern VOID* (__stdcall *XnetUncachedAllocProc)(SIZE_T, ULONG);
extern VOID (__stdcall *XnetUncachedFreeProc)(VOID*);
#ifdef DVTSNOOPBUG
