...

{
    ...
    DbgPrint("xnet-init:  Ip route table ok\n");
    ...
    for (ifindex=0; ifindex < IFINDEX_MAX; ifindex++) {
        ...
        DbgPrint("xnet-init:  iface[%u] init...\n", (unsigned)ifindex);
        status = initProc(&ifp);
        if (!NT_SUCCESS(status)) goto failed;
        DbgPrint("xnet-init:  iface[%u] init done\n", (unsigned)ifindex);
        ...
        DbgPrint("xnet-init:  iface[%u] bcast set\n", (unsigned)ifindex);

        if (IfMcastEnabled(ifp)) {
            status = IfInitMcastGroup(ifp);
            if (!NT_SUCCESS(status)) goto failed;
        }
        DbgPrint("xnet-init:  iface[%u] mcast init\n", (unsigned)ifindex);

        if (IfDhcpEnabled(ifp)) {
            status = DhcpInitialize(ifp);
            if (!NT_SUCCESS(status)) goto failed;
        }
        DbgPrint("xnet-init:  iface[%u] dhcp init\n", (unsigned)ifindex);
    }
    DbgPrint("xnet-init:  IpInitialize done\n");
