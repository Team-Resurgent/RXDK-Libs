...
PRIVATE VOID* __stdcall XnetUncachedAlloc(SIZE_T size, ULONG tag)  /* RXDK: __stdcall (matches DM-boundary pointer type) */
{
    return _XnetAlloc(&XnetUncachedPool, size, tag);
}

PRIVATE VOID __stdcall XnetUncachedFree(VOID* p)
{
