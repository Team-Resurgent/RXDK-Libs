...
NTSTATUS CXnSock::UdpSend(CSocket* pSocket, CSendReq * pSendReq, UINT uiFlags)
{
    ...
    ipaDst = pSendReq->toaddr ? (CIpAddr)pSendReq->toaddr->sin_addr.s_addr : pSocket->_ipaDst;  /* RXDK: both ternary arms CIpAddr (clang ambiguity) */
    ...
    pSendBuf = (CSendBuf *)PacketAlloc(PTAG_CUdpPacket, uiFlags, pSendReq->sendtotal,
                                       sizeof(CSendBuf), (PFNPKTFREE)&CXnSock::UdpPacketFree);
