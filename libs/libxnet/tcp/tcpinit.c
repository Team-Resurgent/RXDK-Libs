...
PRIVATE VOID __stdcall   /* RXDK: kernel-invoked DPC -> __stdcall */
TcpFastTimerProc(
    PKDPC dpc,
    VOID* context,
    VOID* param1,
...

{
    ...
    // Runs at DISPATCH (timer DPC) -- no DbgPrint here (re-entrancy with the xbdm
    // enet/debug channel; DbgPrint goes over the same NIC the bridge transmits on).
...

{
    ...
    KeInitializeDpc(&TcpFastTimerDpc, (PKDEFERRED_ROUTINE) TcpFastTimerProc, NULL);
    KeInitializeTimer(&TcpFastTimer);
    KeSetTimerEx(&TcpFastTimer, dueTime, period, &TcpFastTimerDpc);
    DbgPrint("xnet-init:  Tcp timers set; IpInitialize...\n");
