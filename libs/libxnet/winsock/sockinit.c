...
VOID* (__stdcall *XnetUncachedAllocProc)(SIZE_T, ULONG);   /* RXDK: __stdcall (DM boundary) */
VOID (__stdcall *XnetUncachedFreeProc)(VOID*);
...
/* RXDK: XnetInitialize calls XnetCleanup (defined below) before its definition;
   forward-declare with the WSAAPI (__stdcall) convention so the implicit cdecl
   declaration from the call site doesn't conflict with the stdcall definition. */
...

{
    ...
    if (xnetstate == XNETSTATE_INVALID) {
        ...
        DbgPrint("xnet-init: XnetPoolInit...\n");
        // Initialize our private pool
        status = XnetPoolInit();
        if (!NT_SUCCESS(status)) goto failed;

        DbgPrint("xnet-init: DnsInitialize...\n");
        // Initialize the DNS client
        status = DnsInitialize();
        if (!NT_SUCCESS(status)) goto failed;

        DbgPrint("xnet-init: TcpInitialize (-> Ip -> Enet -> Phy/NIC)...\n");
        // Initialize the TCP/IP stack
        status = TcpInitialize();
        if (!NT_SUCCESS(status)) goto failed;
        DbgPrint("xnet-init: stack ready (status=0x%08x)\n", (unsigned)status);
    ...
    if (wait) {
        IfInfo* ifp;
        DbgPrint("xnet-init: waiting for DHCP address...\n");
