...
CPacket * CXnBase::PacketAlloc(ULONG tag, UINT uiFlags, UINT cbPayload, UINT cbPkt, PFNPKTFREE pfn)
{
    ...
    if (ppkt)
    {
        ppkt->Init(uiFlags, (BYTE *)ppkt + cbPtr, cbPkt - cbPtr, pfn ? pfn : (PFNPKTFREE)&CXnBase::PacketFree);
...
// RXDK: __stdcall is REQUIRED. _dpcTimer is a real kernel DPC (armed via the kernel
// KeSetTimerEx facade -- halw.cpp's user-mode emulation is XNET_FEATURE_WINDOWS-only),
// so the kernel invokes this at DISPATCH with stdcall (ret 16). A default-cdecl static
// member would leave the kernel DPC dispatcher's stack 16 bytes off -> EIP=0 bugcheck
// when the timer first fires. See [[dpc-callback-stdcall-convention]].
void __stdcall CXnBase::TimerDpc(PKDPC pkdpc, void * pvContext, void * pvParam1, void * pvParam2)
{
...
NTSTATUS CXnBase::BaseInit(XNetInitParams * pxnip)
{
    ...
    KeInitializeDpc(&_dpcTimer, (PKDEFERRED_ROUTINE)TimerDpc, this);
