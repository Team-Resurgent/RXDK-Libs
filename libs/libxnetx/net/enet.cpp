...
NTSTATUS CXnEnet::EnetInit(XNetInitParams * pxnip)
{
    ...
    KeInitializeDpc(&_dpcEnet, (PKDEFERRED_ROUTINE)&EnetDpc, this);

#ifdef XNET_FEATURE_ARP
    _paeLast = _aae;
    _timerArp.Init((PFNTIMER)&CXnEnet::ArpTimer);
#endif
...
// RXDK: __stdcall REQUIRED -- real kernel DPC (queued via KeInsertQueueDpc, kernel
// invokes it at DISPATCH stdcall). Cdecl would corrupt the DPC dispatcher stack on the
// first RX. See [[dpc-callback-stdcall-convention]] and CXnBase::TimerDpc.
void __stdcall CXnEnet::EnetDpc(PKDPC, void * pEnet, void *, void *)
{
