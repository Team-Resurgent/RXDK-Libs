...
/* RXDK: the naked body does `ret 4` (callee-cleans the 4-byte arg), so the
   function MUST be __stdcall -- otherwise clang's default-cdecl caller ALSO pops
   the arg (add esp,4) and the stack ends up 4 bytes off, corrupting the caller's
   frame (crashed XnInit right after HalQueryTsc). */
__declspec(naked) static void __stdcall QueryTsc(LARGE_INTEGER * pli)
{
    _asm {
        mov ecx, [esp+4]
        rdtsc
