...

{
    ...
    pkt = TcpPacketAlloc(pTcpSocket, pTcpSocket->_ipaDst, synAck ? PKTF_POOLALLOC : 0, SYNTCPOPTLEN,
                         sizeof(CTcpSendBuf), (PFNPKTFREE)&CXnSock::TcpPacketFree);
    if (!pkt)
    {
...
{
    ICHECK(SOCK, UDPC);
    CPacket * ppkt = TcpPacketAlloc(pTcpSocket, pTcpSocket->_ipaDst, 0, 0, sizeof(CTcpSendBuf), (PFNPKTFREE)&CXnSock::TcpPacketFree);
...

{
    ...
    while (bytesToSend > 0)
    {
        ...
        pkt = TcpPacketAlloc(pTcpSocket, pTcpSocket->_ipaDst, uiFlags, segsize, sizeof(CTcpSendBuf), (PFNPKTFREE)&CXnSock::TcpPacketFree);
