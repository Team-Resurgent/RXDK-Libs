...
/*
 * Differential compression and online update patching -- applies binary diffs to
 * installed content.
 */
			...
			)
{
	...
	else
	{
		...
		pautoupd->dircopy.xontaskOnFile.pfnContinue = &CXo::UpdateOnlineCountFilesContinue;
			...
			)
{
	...
	pcontdl->xrlasync.xrlext.dl.pfnCleanup = &CXo::AutoupdateCleanup;

	// Set up the autoupdate context
	TaskInitializeContext(&pautoupd->xontask);
	pautoupd->xontask.pfnContinue = &CXo::xoupdContinue;
