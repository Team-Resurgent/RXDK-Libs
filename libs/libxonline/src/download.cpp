			...
			)
{
	...
	pxrlasync->xontask.pfnContinue = &CXo::xrldlContinue;
	pxrlasync->xontask.pfnClose = &CXo::XRL_MainClose;
			...
			)
{
	...
	pxrldlext->pfnDoneHeaders = &CXo::xrldtmDoneHeaders;
	pxrldlext->pfnProcessData = &CXo::xrldtmProcessData;
	pxrldlext->pfnCompleted = &CXo::xrldtmCompleted;
			...
			)
{
	...
	pxrlasyncext->xontask.pfnContinue = &CXo::xrldfContinue;
	pxrlasyncext->xontask.hEventWorkAvailable = hWorkEvent;
	pxrlasyncext->dwCurrentState = xrldfDone;

	// Set up the extension block
	pxrldlext = &(pxrlasync->xrlext.dl);
	pxrldlext->pfnDoneHeaders = &CXo::xrldfDoneHeaders;
	pxrldlext->pfnProcessData = &CXo::xrldfProcessData;
	pxrldlext->pfnCompleted = &CXo::xrldfCompleted;
	pxrldlext->pfnCleanup = NULL;
	pxrldlext->pfnInitializeTask = &CXo::xrldfInitializeTask;
	pxrldlext->hTaskPreConnect = NULL;
	pxrldlext->hTaskProcessData = 
