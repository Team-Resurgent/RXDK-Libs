// dxt-fps -- an Xbox debug-monitor extension (DXT) that draws an FPS / memory /
// temperature overlay using the NV2A PVIDEO hardware video overlay.
//
...
// is a raw PE loaded by xbdm.dll from E:\dxt at debug-monitor init and entered
// at DxtEntry (see private/ntos/dm in the leaked kernel source). It runs in the
// system process, kernel-mode, so it calls the kernel (xboxkrnl.exe) and the
// debug monitor (xbdm.dll) directly -- no XAPI, no D3D, no CRT startup.
//
// Build: links libxbdm.lib (Dm* by ordinal) + libkernel.lib (xboxkrnl.exe by
// ordinal) + libc (sprintf/str*). Produces dxt-fps.dxt; no imagebld, no ISO.
