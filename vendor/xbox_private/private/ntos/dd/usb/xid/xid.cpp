...
#ifdef DEBUG_MOUSE
USB_DEVICE_TYPE_TABLE_ENTRY(XDEVICE_TYPE_DEBUG_MOUSE),
#endif
...
#ifdef DEBUG_MOUSE
        handleCount +=
            XID_TypeInformationList[XID_DEVTYPE_MOUSE].bRemainingHandles =
                UsbInit->GetMaxDeviceTypeCount(XDEVICE_TYPE_DEBUG_MOUSE);
        RIP_ON_NOT_TRUE_WITH_MESSAGE(XID_TypeInformationList[XID_DEVTYPE_MOUSE].bRemainingHandles <= XGetPortCount(),
                                                "XInitDevices: requested more XDEVICE_TYPE_DEBUG_MOUSE than available ports.");
#endif
    } else
    {
        handleCount = XID_TypeInformationList[XID_DEVTYPE_GAMECONTROLLER].bRemainingHandles;
...
#ifdef DEBUG_MOUSE
        handleCount += XID_TypeInformationList[XID_DEVTYPE_MOUSE].bRemainingHandles;
#endif
    }
...
#ifdef DEBUG_MOUSE
        if (XID_DEVTYPE_MOUSE == xidNode->Type)
        {
            if (!XID_MouseOpen(openDevice))
            {
